...
Data (September 16): db.py supports SQLite (default) and PostgreSQL via DATABASE_URL; keep SQL portable (`?` placeholders, quote the "user" column, ON CONFLICT upserts) and run the suite on both. Uploads are stored in the database table `uploads`. Tax: domain.tax_for implements configurable Ethiopian VAT/TOT (server-authoritative; tips untaxed); never present Encore receipts as fiscal receipts.
