...
Data (September 23): db.py is PostgreSQL-only via DATABASE_URL — no SQLite, no file fallback. Keep the `?` placeholder style the adapter translates, quote the "user" column, use ON CONFLICT upserts; `npm test` starts a throwaway PostgreSQL (pgserver). Uploads are stored in the database table `uploads`. Tax: domain.tax_for implements configurable Ethiopian VAT/TOT (server-authoritative; tips untaxed); never present Encore receipts as fiscal receipts.
